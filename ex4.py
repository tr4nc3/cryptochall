...
class CFileParser:
	def __init__(self,fname):
		self.filename = fname
		self.hfile = None
	def findxorencryptedstring(self):
		self.hfile = open(self.filename,"r")
		i = 0
		for line in self.hfile:
			b = CXorDecrypter(line)
			b.calculate()
			print "%d: Keyguess: %d, Plaintext: %s" % (i,b.keyguess(),b.getplain())
			i += 1
def main(argv=None):
	if argv is None:
		argv = sys.argv
	try:
		try:
			x = CFileParser(argv[1])
			x.findxorencryptedstring()
		except IOError:
